...
#                if match:
#                    size = match.group(1) if len(match.groups()) > 2 else '1'
#                    name = match.group(1) if input_type in ['parameter', 'localparam'] else match.group(len(match.groups()) - 1)
#                    default_value = match.group(2) if input_type in ['parameter', 'localparam'] else None
#                    description = description_format.format(size=size.strip() if isinstance(size, str) else size, name=name, default_value=default_value)
#                    
